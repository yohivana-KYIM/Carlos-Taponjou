            ...
            <motion.div variants={fadeUp} className="mt-14">
              <div className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-2 auto-rows-max">
                {t.experience.items.map((exp, i) => {
                  ...
                  return (
                    <Card key={exp.company} className="h-full flex flex-col overflow-hidden hover:shadow-lg transition-shadow">
                      {/* Top visual indicator */}
                      <div className="h-1 bg-gradient-to-r from-primary via-primary/50 to-transparent" />
                      
                      <CardHeader className="bg-gradient-to-br from-primary/10 to-black/[0.02] pb-4 pt-6">
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div className="flex gap-2">
                            {hasPowerBI && (
                              <div className="inline-flex items-center gap-1 rounded-full bg-purple-500/20 px-2.5 py-1.5 border border-purple-500/30">
                                <BarChart3 className="h-3.5 w-3.5 text-purple-500" />
                                <span className="text-xs font-semibold text-purple-600">Power BI</span>
                              </div>
                            )}
                            {hasML && (
                              <div className="inline-flex items-center gap-1 rounded-full bg-cyan-500/20 px-2.5 py-1.5 border border-cyan-500/30">
                                <BrainCircuit className="h-3.5 w-3.5 text-cyan-500" />
                                <span className="text-xs font-semibold text-cyan-600">Data</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <CardTitle className="mt-2.5 text-lg font-medium tracking-tight">
                          {exp.role}
                        </CardTitle>
                      ...
                      <CardContent className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="mt-4 flex flex-wrap gap-1.5 border-t border-foreground/10 pt-4">
                          {experienceTech[i]?.map((tech) => (
                            <span
                              key={tech}
                              className="rounded-full bg-black/[0.04] px-2.5 py-1 text-xs text-muted-foreground hover:bg-black/[0.08] transition-colors"
                            >
                              {tech}
                            </span>
                          ))}
